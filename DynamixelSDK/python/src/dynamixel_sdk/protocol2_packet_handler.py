...
class Protocol2PacketHandler(object):
    ...
    def rxPacket(self, port):
        ...
        while True:
            ...
            if rx_length >= wait_length:
                ...
                if port.isPacketTimeout():
                    #print("ERROR OCURRED HERE BECAUSE OF PACKET TIMEOUT!!")
                    if rx_length == 0:
                        result = COMM_RX_TIMEOUT
                    else:
                        result = COMM_RX_CORRUPT
    ...
    def txRxPacket(self, port, txpacket):
        rxpacket = None
        error = 0
        # print(f"TX PACKET AGAIN:{txpacket}")
        # tx packet
    ...
    def readTxRx(self, port, dxl_id, address, length):
        ...
        rxpacket, result, error = self.txRxPacket(port, txpacket)
        #print(f"TX PACKET: {txpacket}")
        #print(f"RX PACKET: {rxpacket} Result: {result} Error: {error}" )
        if result == COMM_SUCCESS:
            error = rxpacket[PKT_ERROR]
    ...
    def read4ByteTxRx(self, port, dxl_id, address):
        #used for reading position and velocity
    ...
    def writeTxRx(self, port, dxl_id, address, length, data):
        ...
        #print(f"TX PACKET WRITE {txpacket}")
        #print(f"RX PACKET WRITE {rxpacket}")
