...
def storeData():
    ...
    """
    np.savetxt('data/admittance/SensorT',valuesTf,delimiter=",")
    np.savetxt('data/admittance/SensorP',valuesPf,delimiter=",")
    np.savetxt('data/admittance/time3',timevec,delimiter=",")
    np.savetxt('data/admittance/Measured_posP',posP,delimiter=",")
    np.savetxt('data/admittance/Measured_posT',posT,delimiter=",")
...
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler_T, DXL_ID_T, ADDR_OPERATING_MODE, 3)# 1 is velocity control mode
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
...
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler_P, DXL_ID_P, ADDR_GOAL_PWM, 200)# should be 10 otherwise trajectory is not followed
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))

dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler_T, DXL_ID_T, ADDR_GOAL_PWM, 200)# should be 10 otherwise trajectory is not followed
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
...
valuesf=[]
hxP = HX711(8, 7)#Sensor B
hxT = HX711(5, 6)#SensorA
valsT=[0,0,0,0,0]
valsP=[0,0,0,0,0]
referenceUnit = 1
...
while 1:
    try:
        ...
        avpos=0.5*(position_T+position_P)

        newposT=np.int32(avpos+200*valT)
        newposP=np.int32(avpos+200*valP)
        ...
        if(valT+valP<=-0.25):
            ...
            velocity=np.int32(100*(valT+valP))
        ...
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler_P, DXL_ID_P, ADDR_GOAL_POSITION, newposP)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))

        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler_T, DXL_ID_T, ADDR_GOAL_POSITION, newposT)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
