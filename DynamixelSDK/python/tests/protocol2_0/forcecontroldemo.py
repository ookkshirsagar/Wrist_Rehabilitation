...
def storeData():
    np.savetxt('data/versuch5/torque',valuesf,delimiter=",")
    np.savetxt('data/versuch5/vel',vel,delimiter=",")
    np.savetxt('data/versuch5/pwm',pwm,delimiter=",")
    np.savetxt('data/versuch5/pos',pos,delimiter=",")
    np.savetxt('data/versuch5/current',current,delimiter=",")
...
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler_T, DXL_ID_T, ADDR_OPERATING_MODE, 16)# 16 is pwm mode
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
...
while 1:
    ...
    try:
        ...
        dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler_T, DXL_ID_T, ADDR_GOAL_PWM, new_pwm)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
        ...
        current.append(np.int16(current_T)*2.69/1000)
